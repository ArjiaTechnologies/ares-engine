...
def test_equal_score_respects_margin_semantics(tmp_path: Path) -> None:
    strict = GateConfig(min_promotion_score_improvement=0.05)
    first = make_fake_bundle(tmp_path, "first", score=1.0)
    ...
    permissive = GateConfig(min_promotion_score_improvement=0.0)
...
def test_concurrent_promotions_from_two_processes_leave_a_single_coherent_champion(
    ...
    script = textwrap.dedent(
        ...
        try:
            decision = promote(root / sys.argv[1], root, GateConfig(min_promotion_score_improvement=0.0))
            print("PROMOTED")
